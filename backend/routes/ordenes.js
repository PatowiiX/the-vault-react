    ...
    db.beginTransaction(async (err) => {
        ...
        try {
            ...
            for (const item of items) {
                // Consultamos el stock actual (usando promesas para evitar el callback hell)
                ...
                // Registrar en stock_history (según tu dump de BD)
                await db.promise().query(
            ...
            const [resOrden] = await db.promise().query(queryOrden, [
                usuario_id, 
                total, 
            ...
            // Esto es vital para que no le vuelvan a aparecer los productos al recargar
            await db.promise().query("DELETE FROM carrito WHERE usuario_id = ?", [usuario_id]);

            // 4. FINALIZAR TRANSACCIÓN (COMMIT)
            db.commit(async (err) => {
                if (err) throw err;

                console.log(`✅ Pedido #${ordenId} completado con éxito.`);

                // 5. ENVIAR EMAIL DE CONFIRMACIÓN (Async)
